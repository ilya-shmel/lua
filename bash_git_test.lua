...
-- Временная проверка
        local parts = {
          command_executed or "no command",
          (is_git == nil) and "false" or tostring(is_git),
          (shell_command == nil) and "false" or tostring(shell_command),
        }
...
-- Временная проверка
    for _, event in ipairs(events) do
        local parts = {
          event:gets("observer.event.id", "") or "",
          event:gets("observer.event.type", "") or "",
          event:gets("initiator.command.executed", "") or "",
          event:gets("initiator.process.path.name", "") or "",
        }
        log(table.concat(parts, " - "))
    end
--
