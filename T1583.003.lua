...
function on_matched(grouped, matchedData)
    ...
    alert({template = [[Подозрение на компрометацию сторонней инфраструктуры.
...
local pattern = {
    {field = "target.image.name", values = {"powershell.exe"}, count =1},
    {field = "target.image.name", values = {"systeminfo.exe"}, count =1},
    {field = "target.image.name", values = {"taskkill.exe"}, count =1},
    {field = "target.image.name", values = {"powershell.exe"}, count =1},
    {field = "target.image.name", values = {"ping.exe", "PING.EXE"}, count =1},
    {field = "target.image.name", values = {"net.exe"}, count =1}
}

grouper = grouper.new_pattern_matcher(
    {"observer.host.ip", "observer.host.hostname", "observer.host.fqdn"},
    {"observer.host.ip", "observer.host.hostname", "observer.host.fqdn"}, {"@timestamp"},
    pattern,
    "@timestamp,RFC3339",
    "1m",
    on_matched
