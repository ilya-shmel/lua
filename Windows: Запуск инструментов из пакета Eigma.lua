-- Шаблон алерта
local template = [[
Подозрение на запуск скрипта/библиотеки из пакета Enigma.
...
function on_grouped(grouped)
    ...
    if unique_events > 1 then
        ...
        if log_scriptblock and log_module then 
            ...
            local file_name = command_executed:match("([^/]+%.ps1)[\'\"%)]") or command_executed:match(";%s*([^%s;]+)%s+-%w+") or "Имя файла не определено"
