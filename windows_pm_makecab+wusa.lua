...
function on_matched(grouped, matchedData)
	...
	alert({template = [[Подозрение на подмену библиотек DLL с помощью системных утилит в обход UAC
...
{{ end }}]], risk_level = 8.0, asset_ip = logline:get_asset_data("observer.host.ip"), asset_hostname = logline:get_asset_data("observer.host.hostname"), asset_fqdn = logline:get_asset_data("observer.host.fqdn"), asset_mac = logline:get_asset_data(""), create_incident = true, incident_group = "", assign_to_customer = false, incident_identifier = "", logs = matchedData.loglines, mitre = {"T1080"}, trim_logs = 5})
end

local pattern = {
    {field = "target.image.name", values = {"makecab.exe"}, count = 1},
    {field = "target.image.name", values = {"wusa.exe"}, count = 1},
    {field = "target.image.name", values = {"makecab.exe"}, count = 1},
    {field = "target.image.name", values = {"wusa.exe"}, count = 1}
}

grouper = grouper.new_pattern_matcher({"observer.host.ip", "observer.host.hostname", "observer.host.fqdn"}, {"observer.host.ip", "observer.host.hostname", "observer.host.fqdn"}, {"@timestamp"}, pattern, "@timestamp,RFC3339", "10m", on_matched)