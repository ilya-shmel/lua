...
local function analyze_syscall(path)
    ...
    local regular3 = sys_path:search(bins)
    local regular4 = sys_path:search(shell_pattern)
    if regular3 or regular4 then
       return true
    end
end

-- Функция поиска строк, похожих на файл
local function like_filename(string1)
    string1 = string1:match('^"?([^"?]+)"?$')
    string1 = string1:match("([^%?]+)") -- Защита от знака `?`
    local base = string1:match("([^/]+)$")
    -- Считаем именем то, что не начинается с '-' и содержит хотя бы одну букву/цифру
    return base and base ~= "" and not base:match("^%-") and base:match("%w") and base  --убрал or nil в конце
end

-- Функция поиска точного имени файла
local function extract_filename(string2)
    -- Ищем параметры -O/-o
    local output = string2:match("%-[Oo]%s*([^%s]+)")
    if output then
      output = output:match('^"?([^"?]+)"?$')
      output = output:match("([^%?]+)")
      return output:match("([^/]+)$")
    ...
    local best_match
    for command in string2:gmatch("%S+") do
      local file = like_filename(command)
      if file then 
        best_match = file
      end
    end
...
function on_grouped2(grouped)
    ...
    if grouped.aggregatedData.unique.total > 1 then
        for _, event in ipairs(events) do
            if event:gets("observer.event.type") == "EXECVE" and event:gets("operation.type") == "download" then
            ...
            elseif event:gets("observer.event.type") == "EXECVE" and event:gets("operation.type") == "execution" then
