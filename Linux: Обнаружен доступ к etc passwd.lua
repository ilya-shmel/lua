-- Шаблон алерта
local template = [[
	Подозрение на несанкционированный доступ к файлу /etc/passwd.
	Узел: 
...
local regex_patterns = {
    "(?:^|\\s+|\\/|\"|\')(test)?cat\\s+(\\/etc\\/)?passwd\\s+>(>)?\\s+(\\/[^\\/ ]*)+",
    "(?:^|\\s+|\\/|\"|\')sed\\s+(\"|\'){2}\\s+(\\/etc\\/)?passwd\\s+>{1,2}\\s+(\\/[^\\/ ]*)+\\/?",
    "(?:^|\\s+|\\/|\"|\')awk\\s+\\'{\\w+}\\'\\s+(\\/etc\\/)?passwd\\s+>{1,2}\\s+(\\/[^\\/ ]*)+\\/?",
    "(?:^|\\s+|\\/|\"|\')tee\\s+(\\/[^\\/ ]*)+\\/?\\s+<\\s+(\\/etc\\/)?passwd",
    "(?:^|\\s+|\\/|\"|\')cat\\s+(\\/etc\\/)?passwd\\s+\\|\\s+tee\\s+(\\/[^\\/ ]*)+\\/?\\s+>{1,2}(\\/[^\\/ ]*)+\\/?",
    "(?:^|\\s+|\\/|\"|\')dd\\s+if=(\\/etc\\/)?passwd\\s+of=(\\/[^\\/ ]*)+\\/?\\s+(\\w+=\\w+(\\s+)?){1,5}",
    "(?:^|\\s+|\\/|\"|\')rsync\\s+(-{1,2}\\w+(=\\w+)?\\s+){1,10}(\\/etc\\/)?passwd(\\s+)?(\\/[^\\/\\s]+)*",
    "(?:^|\\s+|\\/|\"|\')tar\\s+([-\\w\\s\"$\\(\\)]+(?:(\\/etc\\/passwd)?|passwd)){1,2}",
    "(?:^|\\s+|\\/|\"|\')(?:(n)?vi(m)?|nano|emacs|micro|jed|joe|mcedit|pico)\\s+(\\/etc\\/)?passwd"
    }
