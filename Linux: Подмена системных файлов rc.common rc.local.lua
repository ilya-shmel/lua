-- Шаблон алерта
local template = [[
	Обнаружена правка/подмена оригинального системного файла rc.common/rc.local.
	Узел: 
...
local regex_patterns = {
    "(?:^|\\s+|\\/|\"|\')tee\\s+((-{1,2}[\\w=\\,\\s]+){1,10}){1,10}(\\/[^\\/ ]*)+\\/rc\\.(common|local)",
    "(?:^|\\s+|\\/|\"|\')cp\\s+((-{1,2}[\\w=\\,\\s]+){1,10}){0,10}(\\/[^\\/ ]*)+\\/rc\\.(common|local)\\s+(\\/[^\\/ ]*)+\\/[\\w\\.]+",
    "(?:^|\\s+|\\/|\"|\')echo\\s+((-{1,2}[\\w=\\,\\s]+){1,10}){0,10}(?:\'|\")?(.*)(?:\'|\")?\\s+>{1,2}\\s+((\\/[^\\/ ]*)+\\/)?rc\\.(common|local)",
    "(?:^|\\s+|\\/|\"|\')(?:sed|perl)\\s+((-{1,2}[\\w=\\,\\s,\\.]+){1,10}){1,10}(?:\'|\")?\\w+\\/.*\\/.*\\/\\w+(?:\'|\")?\\s+((\\/[^\\/ ]*)+\\/)?rc\\.(common|local)",
    "(?:^|\\s+|\\/|\"|\')awk\\s+(?:\'|\")([\\w={}$\"\'\\s]+)(?:\'|\")\\s+((\\/[^\\/ ]*)+\\/)?rc\\.(?:common|local)",
    "(?:^|\\s+|\\/|\"|\')(?:ed|ex)\\s+((-{1,2}[\\w=\\,\\s,\\.]+){1,10}){0,10}(?:\\+|-)(?:\"|\')?\\/.*\\/;\\w+=\\w+(?:\"|\')?\\s+((-{1,2}[\\w=\\,\\s,\\.\'\"]+){1,10}){0,10}((\\/[^\\/ ]*)+\\/)?rc\\.(common|local)"
}
